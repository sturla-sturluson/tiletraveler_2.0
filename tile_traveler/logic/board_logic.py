...
class TileLogic:
    ...
    def __create_board(self):
        ...
        for x in range(self.map_height):
            ...
            self.__add_tile_row(y_converted, self.map_width)

    def __get_border_list(self, y: int, x: int) -> list[Borders]:
        """Gets a list of all the edge borders for the tile position"""
        border_list = list()
        border_list.append(self.__get_north_border(y))
        border_list.append(self.__get_south_border(y))
        border_list.append(self.__get_west_border(x))
        border_list.append(self.__get_east_border(x))
        return border_list

    def __add_tile_row(self, y_converted: int, map_width: int):
        """Loops through the map width and creates the tiles and adds them to the tile dict"""
        for x in range(map_width):
            x_converted = x + 1
            border_list = self.__get_border_list(y_converted, x_converted)

            self.tile_dict[x_converted, y_converted] = Tile(
                (x_converted, y_converted), *border_list
            )

    def __get_north_border(self, y_pos: int) -> Borders:
